...
import { deleteFile, renameFile, starFile, trashFile } from "@/API/Firestore"; // Ensure these methods are compatible with your Django API
import { useRouter } from "next/router";

interface FileDropDownProps {
  file: { id: string; fileName: string; fileLink: string; isStarred: boolean; isFolder: boolean; }; // Adjust the type as needed
  setOpenMenu: (value: string) => void;
  select: string;
  isFolderComp: boolean;
  folderId: string;
  setRenameToggle: (id: string) => void;
}
      ...
      {select !== "trashed" ? (
        <>
          <div
            onClick={() =>
              isFolderComp
                ? router.push("/drive/directories/" + folderId) // Change 'folders' to 'directories'
          ...
          <div
            onClick={() => starFile(file.id, !file.isStarred)} // Ensure starFile is correctly set up in your API
            className="my-2 flex items-center space-x-3 px-3 py-1.5 hover:cursor-pointer hover:bg-[#ddd]"
          >
            {!file.isStarred ? (
          ...
          <div
            onClick={() => trashFile(file.id, true)} // Ensure trashFile interacts correctly with your API
            className="my-2 flex items-center space-x-3 px-3 py-1.5 hover:cursor-pointer hover:bg-[#ddd]"
          >
        ...
        <>
          <div
            onClick={() => trashFile(file.id, false)} // Ensure this is properly implemented
            className="my-2 flex items-center space-x-3 px-3 py-1.5 hover:cursor-pointer hover:bg-[#ddd]"
          >
          ...
          <div
            onClick={() => deleteFile(file.id, file.isFolder)} // Ensure deleteFile is implemented correctly
            className="my-2 flex items-center space-x-3 px-3 py-1.5 hover:cursor-pointer hover:bg-[#ddd]"
          >
            <RiDeleteBin6Line className="h-5 w-5" />
